...
def test_an_error_is_raised_if_dictionary_is_empty(
    validation, empty_dictionary
) -> None:
    with pytest.raises(ValidationError):
        validation.validate_dictionary(empty_dictionary)


def test_an_error_is_not_raised_if_dictionary_is_valid(
    validation, test_dictionary
) -> None:
    try:
...
def test_an_error_is_raised_for_invalid_start_words(
    validation: Validation, start_word: str, test_graph: nx.Graph
) -> None:
    with pytest.raises(ValidationError):
...
def test_an_error_is_raised_for_invalid_end_words(
    validation: Validation, end_word: str, test_graph: nx.Graph
) -> None:
    with pytest.raises(ValidationError):
...
def test_we_can_validate_both_words_at_once(
    validation: Validation, invalid_word: str, test_graph: nx.Graph
) -> None:
    with pytest.raises(ValidationError):
...
def test_no_errors_are_raised_for_valid_words(
    validation: Validation, valid_word: str, test_graph: nx.Graph
) -> None:
    try:
